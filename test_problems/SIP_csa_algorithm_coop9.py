...
class CSA:
    ...
    def __init__(self, parse_input):
        ...
        self.delta_lb = 0
        self.delta_ub = 1
        self.mh_algorithm_variance = 1.5

        # ========= calculate the parameters ========= #
        self.l_g_x = 6 / np.sqrt(6)  # given any delta, L-constant of g
        self.l_f = 1.32  # sup|gradient(f)|
        self.l_g_delta = 5 / np.sqrt(2)  # ## given any x, L-constant of g(x, delta)
    ...
    def adaptive_constraint_sampling(self, x, k=0, iteration=800):
        ...
        theta_rand = np.random.normal(0, self.mh_algorithm_variance, [self.delta_dim, iteration])
    ...
    def calculate_g_grad_and_values_arr(self, x, theta_rand_value):
        g_values = np.exp(theta_rand_value[0, :] ** 2 + theta_rand_value[1, :] ** 2) \
                   - x[0] \
                   - theta_rand_value[0, :] * x[1] \
                   - theta_rand_value[1, :] * x[2] \
                   - theta_rand_value[0, :]**2 * x[3] \
                   - theta_rand_value[0, :] * theta_rand_value[1, :] * x[4] \
                   - theta_rand_value[1, :]**2 * x[5]  # values in abs
        g_grad = np.array([- np.ones(len(theta_rand_value[0, :])),
                           - theta_rand_value[0, :],
                           - theta_rand_value[1, :],
                           - theta_rand_value[0, :]**2,
                           - theta_rand_value[0, :] * theta_rand_value[1, :],
                           - theta_rand_value[1, :]**2])
    ...
    def calculate_f_grad(self, x_sol):
        f_grad = np.array([1, 1/2, 1/2, 1/3, 1/4, 1/3])
        return f_grad

    def calculate_f_values(self, x_sol):
        f_values = np.dot(np.array([1, 1/2, 1/2, 1/3, 1/4, 1/3]), x_sol)
        return f_values
...
def main():
    ...
    parse_input = {
        ...
        'c_gamma': 0.2,  # 0.03
        'c_eta': 0.001,  # 0.0005
        'x0': np.ones(6),  # according to the paper, the third one is eta, whose maximum is 5.389 if x=(1,1)
        'mcmc_range_factor': 1
    }
    csa = CSA(parse_input)
